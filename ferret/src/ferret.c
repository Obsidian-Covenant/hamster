...
ferret_snarf(struct Ferret *ferret, const char *filename, const unsigned char *px, unsigned length)
{
	...
	if (fp == NULL) {
	    ...
	    n = snprintf(newfile, sizeof(newfile), "%s/%s",
	                 ferret->snarfer.directory, filename);

	    if (n < 0) {
	        fprintf(stderr, "snarf: snprintf failed\n");
	        return;
	    }
	    if ((size_t)n >= sizeof(newfile)) {
	        fprintf(stderr, "snarf: path too long: dir='%s' file='%s'\n",
	                ferret->snarfer.directory, filename);
	        return;
	    }
	}
