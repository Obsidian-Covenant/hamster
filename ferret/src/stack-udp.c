...
static unsigned
smellslike_udp_bootp(struct Ferret *ferret, struct NetFrame *frame,
                     ...
                     unsigned direction)
{
    ...
    /* The elapsed time must be less than 5 minutes (best-effort heuristic) */
    if (length >= 10) {
...
{
    /* Must at least fit BOOTP header (236) + DHCP magic cookie (4) */
    if (length < 240)
        return false;

    /* Basic BOOTP sanity first (re-use your existing heuristic) */
    if (!smellslike_udp_bootp(ferret, frame, px, length, direction))
        return false;

    /* DHCP magic cookie at offset 236 */
    if (ex32be(px + 236) != 0x63825363)
        return false;
...
void process_udp(struct Ferret *ferret, struct NetFrame *frame, const unsigned char *px, unsigned length)
{
	...
	if (udp.src_port != 67 && udp.src_port != 68 && udp.dst_port != 67 && udp.dst_port != 68) {
	    if (frame->layer7_protocol == 0) {
	        const unsigned char *payload = px + offset;
	        unsigned payload_length = length - offset;

	        if (smellslike_udp_dhcp(ferret, frame, payload, payload_length, SMELLS_DST) ||
	            smellslike_udp_dhcp(ferret, frame, payload, payload_length, SMELLS_SRC) ||
	            smellslike_udp_bootp(ferret, frame, payload, payload_length, SMELLS_DST) ||
	            smellslike_udp_bootp(ferret, frame, payload, payload_length, SMELLS_SRC)) {

	            process_dhcp(ferret, frame, payload, payload_length);
	            return;
	        }
	    }
	}
