...
static int
sdp_next_header(char name, const unsigned char *px, unsigned length,
                struct Field *field, unsigned index)
{
    unsigned offset = 0;
    unsigned count = 0;

    while (offset < length) {
        unsigned i, line_length, value_offset, next_offset;

        for (i = 0; offset + i < length && px[offset + i] != '\n'; i++)
            ;
        next_offset = (offset + i < length) ? offset + i + 1 : length;
        ...
        /* must be "x=" */
        if (line_length < 2 || px[offset] != (unsigned char)name || px[offset + 1] != '=') {
            offset = next_offset;
            continue;
        }
...
*/
	...
	while (sdp_next_header('m', px, length, field, index)) {
		...
		if (protocol_type->length) {
			...
			(void)field_next_number(port_field, &offset2);
		}
