...
void myprox_ferret_thread(void *v_parms)
{
	...
	int n = pixie_snprintf(args, sizeof(args), "-i %s --hamster",
	                       myprox->adapters[i].adaptername);
	if (n < 0 || (size_t)n >= sizeof(args)) {
	    fprintf(stderr, "adapter name too long: %s\n", myprox->adapters[i].adaptername);
	    free(v_parms);
	    return;
	}
...
int main(int argc, char *argv[])
{
	...
	printf("--- HAMSTER 2.1 side-jacking tool ---\n");
