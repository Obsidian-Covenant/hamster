 ...
 */
unsigned pixie_get_mac_address(unsigned char macaddr[PIXIE_MAC_LEN])
{
	memset(macaddr, 0, PIXIE_MAC_LEN);
#ifdef WIN32
	{
		DWORD dwStatus;
		IP_ADAPTER_INFO *p;
		...
		int fd_stdin, int fd_stdout)
{
	...
	if ((pid = fork()) == -1) {
		...
		if (dup2(fd_stdin, 0) == -1) {
			...
			/* Execute CGI program */
			execvp(prog, (char *const *)argarray);
		...
		for (unsigned j = 1; j < arg_count; j++)
        	free((void*)argarray[j]);
	}
